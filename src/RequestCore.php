...
class RequestCore
{
    ...
    public function process_response($curl_handle = null, $response = null)
    {
        ...
        if (is_resource($curl_handle))
        {
            ...
            {
                return new ResponseCore($this->response_headers, $this->response_body, $this->response_code, $curl_handle);
            }
        }
    ...
    public function send_request($parse = false)
    {
        ...
        {
            throw new \Exception('cURL resource: ' . (string) $curl_handle . '; cURL error: ' . curl_error
                ($curl_handle) . ' (' . curl_errno($curl_handle) . ')');
        }
    ...
    public function send_multi_request($handles, $opt = null)
    {
        ...
        do
        {
            ...
            while ($done = curl_multi_info_read($multi_handle))
            {
                ...
                if ($done['result'] > 0)
                {
                    throw new \Exception('cURL resource: ' . (string) $done['handle'] . '; cURL error: ' . curl_error($done['handle']) . ' (' . $done['result'] . ')');
                }
